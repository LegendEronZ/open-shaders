...
#		elif defined(MOONMASK)
	...
	// The moon blend color is display-referred and can exceed 1.0; vanilla clips it at output, but a
	// decode amplifies it instead, so cap it to the vanilla ceiling before composing.
	if (composeAuthoredSky)
		psout.Color.xyz = Color::Sky(saturate(psout.Color.xyz));
