...
#		if defined(RENDER_DEPTH)
	...
	const float baseAlpha = TexBaseSampler.SampleBias(SampBaseSampler, alphaUV, SharedData::AlphaTestMipBias).w;
...
#		else
	const float2 baseUV = float2(input.TexCoord.x, input.TexCoord.y * (complex ? 0.5 : 1.0));
	float4 baseColor = TexBaseSampler.SampleBias(SampBaseSampler, baseUV, SharedData::MipBias);

#			if defined(DO_ALPHA_TEST)
	float diffuseAlpha = input.Color.w * AlphaTestBias::SampleAlpha(TexBaseSampler, SampBaseSampler, baseUV, baseColor.w);
	if ((diffuseAlpha - AlphaTestRefRS) < 0) {
		discard;
	}
#			endif
...
#		if defined(RENDER_DEPTH)
	const float baseAlpha = TexBaseSampler.SampleBias(SampBaseSampler, input.TexCoord.xy, SharedData::AlphaTestMipBias).w;
	const float diffuseAlpha = input.Fade * baseAlpha;
	if ((diffuseAlpha - AlphaTestRefRS) < 0) {
		discard;
	}
...
#			if defined(DO_ALPHA_TEST)
	const float diffuseAlpha = input.Color.w * AlphaTestBias::SampleAlpha(TexBaseSampler, SampBaseSampler, input.TexCoord.xy, baseColor.w);
	if ((diffuseAlpha - AlphaTestRefRS) < 0)
		discard;
#			endif
