...
#define I18N_KEY_PREFIX "feature.adaptive_brightness."
...
namespace
{
	...
	{
		a_status += " Use Save Settings to keep these changes after closing the game.";
		return a_status;
	}
	...
	void DrawPresetNameInput(const char* a_label, const char* a_id, std::string& a_name, const std::filesystem::path& a_exportPath, const std::filesystem::path* a_alternateImportPath = nullptr)
	{
		...
		ImGui::InputTextWithHint(a_id, T(TKEY("preset_name_hint"), "Preset name"), &a_name);
		if (auto _tt = Util::HoverTooltipWrapper()) {
			ImGui::Text(T(TKEY("export_path_label"), "Export path: %s"), a_exportPath.string().c_str());
			if (a_alternateImportPath)
				ImGui::Text(T(TKEY("import_alternate_path_label"), "Import also accepts: %s"), a_alternateImportPath->string().c_str());
		}
	}
...
{
	ImGui::Checkbox(T(TKEY("enable"), "Enable Adaptive Brightness"), &settings.enabled);
...
void AdaptiveBrightness::DrawSettings()
{
	...
	ImGui::SeparatorText(T(TKEY("profiles"), "Profiles"));
	if (ImGui::BeginTabBar("##AdaptiveBrightnessProfiles", ImGuiTabBarFlags_None)) {
		for (auto profile : kProfileOrder) {
			...
			if (ImGui::BeginTabItem(GetProfileName(profile).c_str(), nullptr, tabFlags)) {
...
std::string AdaptiveBrightness::GetProfileName(Profile a_profile)
{
	switch (a_profile) {
	case Profile::ExteriorDay:
		return T(TKEY("profile_exterior_day"), "Exterior Day");
	case Profile::ExteriorNight:
		return T(TKEY("profile_exterior_night"), "Exterior Night");
	case Profile::Interior:
		return T(TKEY("profile_interior"), "Interior");
	case Profile::Dungeon:
		return T(TKEY("profile_dungeon"), "Dungeon");
	case Profile::Dwelling:
		return T(TKEY("profile_dwelling"), "Dwelling");
	default:
		return "";
	}
}
...
{
	ImGui::SeparatorText(T(TKEY("exterior_time"), "Exterior Time"));
	ImGui::SliderFloat(T(TKEY("day_blend_start"), "Day Blend Start"), &settings.dayStartHour, 0.0f, 24.0f, "%.1f h");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("day_blend_start_tooltip"), "Hour when the Exterior Day profile starts blending in."));
	}

	ImGui::SliderFloat(T(TKEY("night_blend_start"), "Night Blend Start"), &settings.nightStartHour, 0.0f, 24.0f, "%.1f h");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("night_blend_start_tooltip"), "Hour when the Exterior Night profile starts blending in."));
	}

	ImGui::SliderFloat(T(TKEY("blend_duration"), "Blend Duration"), &settings.transitionHours, 0.0f, 4.0f, "%.1f h");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("blend_duration_tooltip"), "Hours used to blend between Exterior Day and Exterior Night."));
	}
...
void AdaptiveBrightness::DrawProfileSettings(ProfileSettings& a_profile, const char* a_sectionTitle, bool a_showAdvancedControls)
{
	...
	if (a_sectionTitle) {
		...
		if (std::strcmp(a_sectionTitle, "Profile Values") == 0) {
			localizedTitle = T(TKEY("profile_values"), "Profile Values");
		} else if (std::strcmp(a_sectionTitle, "Override Profile Values") == 0) {
			localizedTitle = T(TKEY("override_profile_values"), "Override Profile Values");
		} else {
			localizedTitle = a_sectionTitle;
		}
		ImGui::SeparatorText(localizedTitle.c_str());
	}
	ImGui::Indent();

	ImGui::Spacing();
	ImGui::SliderFloat(T(TKEY("scene_brightness"), "Scene Brightness"), &a_profile.brightness, kBrightnessMin, kBrightnessMax, "%.2f", ImGuiSliderFlags_AlwaysClamp);
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("scene_brightness_tooltip"), "Overall brightness for this profile. Use it when this location type is too dark or too bright."));
	}

	if (a_showAdvancedControls) {
		bool advancedControls = advancedControlsOpen;
		if (ImGui::Checkbox(T(TKEY("advanced_controls"), "Advanced Controls"), &advancedControls))
	...
	if (a_showAdvancedControls && advancedControlsOpen) {
		ImGui::SeparatorText(T(TKEY("light_multipliers"), "Light Multipliers"));
		ImGui::SliderFloat(T(TKEY("directional_light"), "Directional Light"), &a_profile.directionalLightMult, 0.0f, 3.0f, "%.2f");
		ImGui::SliderFloat(T(TKEY("point_lights"), "Point Lights"), &a_profile.pointLightMult, 0.0f, 3.0f, "%.2f");
		ImGui::SliderFloat(T(TKEY("ambient"), "Ambient"), &a_profile.ambientMult, 0.0f, 3.0f, "%.2f");
		ImGui::SliderFloat(T(TKEY("emissive"), "Emissive"), &a_profile.emitColorMult, 0.0f, 3.0f, "%.2f");
		ImGui::SliderFloat(T(TKEY("glowmaps"), "Glowmaps"), &a_profile.glowmapMult, 0.0f, 3.0f, "%.2f");
		ImGui::SliderFloat(T(TKEY("effects"), "Effects"), &a_profile.effectLightingMult, 0.0f, 3.0f, "%.2f");

		ImGui::SeparatorText(T(TKEY("atmosphere_gamma_offsets"), "Atmosphere Gamma Offsets"));
		ImGui::SliderFloat(T(TKEY("sky"), "Sky"), &a_profile.skyGammaOffset, kGammaOffsetMin, kGammaOffsetMax, "%.2f");
		ImGui::SliderFloat(T(TKEY("fog"), "Fog"), &a_profile.fogGammaOffset, kGammaOffsetMin, kGammaOffsetMax, "%.2f");
		ImGui::SliderFloat(T(TKEY("fog_transparency"), "Fog Transparency"), &a_profile.fogAlphaGammaOffset, kGammaOffsetMin, kGammaOffsetMax, "%.2f");
		ImGui::SliderFloat(T(TKEY("water"), "Water"), &a_profile.waterGammaOffset, kGammaOffsetMin, kGammaOffsetMax, "%.2f");
		ImGui::SliderFloat(T(TKEY("volumetric_lighting"), "Volumetric Lighting"), &a_profile.vlGammaOffset, kGammaOffsetMin, kGammaOffsetMax, "%.2f");
	}
...
{
	ImGui::SeparatorText(T(TKEY("global_presets"), "Global Presets"));
	DrawHintText(T(TKEY("global_presets_hint1"), "Global presets store the five profile tabs and exterior timing."));
	DrawHintText(T(TKEY("global_presets_hint2"), "Import overwrites those profile tabs in the current settings. Saved location overrides are not changed."));
	DrawHintText(T(TKEY("imported_changes_save_hint"), "Import changes the current settings immediately, but those changes are not saved automatically. Use the main Save Settings button to keep them after closing the game."));
	ImGui::PushID("GlobalPresetControls");

	const auto presetPath = GetPresetPath(globalPresetName, PresetKind::Global);
	DrawPresetNameInput(T(TKEY("global_preset_label"), "Global preset"), "##GlobalPresetName", globalPresetName, presetPath);

	ImGui::SameLine();
	if (ImGui::Button(T(TKEY("export_global"), "Export Global"))) {
		ExportGlobalPreset();
	}
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("export_global_tooltip"), "Export exterior timing and the five profile tabs. Location overrides are not included."));
	}

	ImGui::SameLine();
	if (ImGui::Button(T(TKEY("import_global"), "Import Global"))) {
		ImportGlobalPreset();
	}
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("import_global_tooltip"), "Replace the five profile tabs in the current settings. Saved location overrides stay unchanged."));
	}
...
{
	ImGui::SeparatorText(T(TKEY("location_override_profiles"), "Location Override Profiles"));
	DrawHintText(T(TKEY("location_override_hint1"), "Location overrides are per-place profiles. A saved override is used when its location or cell matches where you are."));
	if (a_includePresetControls) {
		DrawHintText(T(TKEY("location_override_hint2"), "Import adds overrides from a preset to the override list below. Later edits change this list, not the preset file."));
		DrawHintText(T(TKEY("imported_changes_save_hint"), "Import changes the current settings immediately, but those changes are not saved automatically. Use the main Save Settings button to keep them after closing the game."));
	}
	...
	std::string currentOverrideButtonLabel;
	if (hasSavedTarget) {
		currentOverrideButtonLabel = T(TKEY("open_current_location_override"), "Open Current Location Override");
	} else {
		currentOverrideButtonLabel = T(TKEY("create_current_location_override"), "Create Current Location Override");
	}
	...
	if (ImGui::Button(currentOverrideButtonLabel.c_str())) {
		SaveCurrentLocationOverride();
	}
	ImGui::EndDisabled();

	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("current_override_tooltip"), "Save this place as an override, or open its existing override."));
	}

	ImGui::SameLine();
	ImGui::TextDisabled(T(TKEY("saved_count"), "(%zu saved)"), settings.locationOverrides.size());

	if (target) {
		ImGui::TextWrapped(T(TKEY("current_target"), "Current target: %s (form %s, %s, COC %s)"), target->name.c_str(), target->type.c_str(), target->key.c_str(), GetCocLabel(target->cocCode));
	} else {
		ImGui::TextDisabled("%s", T(TKEY("no_current_location"), "No current location or cell form is available."));
	}

	if (a_includePresetControls)
		DrawLocationOverridePresetControls();
	ImGui::SeparatorText(T(TKEY("saved_overrides"), "Saved Overrides"));
	DrawHintText(T(TKEY("saved_overrides_hint"), "These saved overrides are matched by location or cell. Click a row to edit it."));

	if (settings.locationOverrides.empty()) {
		ClearLocationOverrideSelection();
		ImGui::TextDisabled("%s", T(TKEY("no_overrides_saved"), "No location overrides saved."));
		return;
	}
	...
	if (ImGui::BeginTable("##AdaptiveBrightnessLocationOverrides", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Sortable | ImGuiTableFlags_SizingStretchProp)) {
		const float actionColumnWidth = std::max(
			...
			ImGui::CalcTextSize(T(TKEY("action_edit"), "Edit")).x + ImGui::CalcTextSize(T(TKEY("action_copy"), "Copy")).x + ImGui::GetStyle().FramePadding.x * 4.0f + ImGui::GetStyle().ItemSpacing.x + 8.0f);

		ImGui::TableSetupColumn(T(TKEY("col_location"), "Location"), ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthStretch, 2.0f, ColLocation);
		ImGui::TableSetupColumn(T(TKEY("col_form"), "Form"), ImGuiTableColumnFlags_WidthFixed, 64.0f * Util::GetUIScale(), ColForm);
		ImGui::TableSetupColumn(T(TKEY("col_key"), "Key"), ImGuiTableColumnFlags_WidthStretch, 1.6f, ColKey);
		ImGui::TableSetupColumn(T(TKEY("col_coc"), "coc"), ImGuiTableColumnFlags_WidthStretch, 2.0f, ColCoc);
		ImGui::TableSetupColumn(T(TKEY("col_actions"), "Actions"), ImGuiTableColumnFlags_NoSort | ImGuiTableColumnFlags_WidthFixed, actionColumnWidth, ColActions);
		...
		for (const auto overrideIndex : sortedIndices) {
			...
			if (auto _tt = Util::HoverTooltipWrapper()) {
				if (locationOverride.cocCode.empty()) {
					ImGui::Text("%s", T(TKEY("no_cell_editorid_tooltip"), "No cell EditorID was saved for this override."));
				} else {
					ImGui::Text(T(TKEY("console_coc_tooltip"), "Console: coc %s"), locationOverride.cocCode.c_str());
				}
			}

			ImGui::TableSetColumnIndex(4);
			if (ImGui::SmallButton(T(TKEY("action_edit"), "Edit"))) {
				selectedLocationOverrideKey = locationOverride.key;
			}
			ImGui::SameLine();
			ImGui::BeginDisabled(locationOverride.cocCode.empty());
			if (ImGui::SmallButton(T(TKEY("action_copy"), "Copy"))) {
				const auto command = std::format("coc {}", locationOverride.cocCode);
				ImGui::SetClipboardText(command.c_str());
			}
			ImGui::EndDisabled();
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T(TKEY("copy_coc_tooltip"), "Copy the coc command for this saved cell."));
			}
			if (ImGui::SmallButton(T(TKEY("action_delete"), "Delete"))) {
	...
	if (auto* selectedOverride = FindLocationOverride(selectedLocationOverrideKey)) {
		ImGui::SeparatorText(T(TKEY("edit_override_profile"), "Edit Location Override Profile"));
		...
		if (auto* editProfile = GetLocationOverrideEditProfile(*selectedOverride)) {
			...
			if (ImGui::Button(T(TKEY("action_save_edit"), "Save Edit"))) {
				selectedOverride->profile = *editProfile;
				ClearLocationOverrideSelection();
			}
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T(TKEY("save_edit_tooltip"), "Save these values to the selected override."));
			}
			ImGui::SameLine();
			if (ImGui::Button(T(TKEY("action_cancel"), "Cancel"))) {
				ClearLocationOverrideSelection();
			}
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T(TKEY("cancel_edit_tooltip"), "Discard changes to the selected override."));
			}
		}
		ImGui::PopID();
...
{
	ImGui::SeparatorText(T(TKEY("override_presets"), "Override Presets"));
	DrawHintText(T(TKEY("override_presets_hint"), "Override presets store saved location and cell overrides only. They do not include the five profile tabs."));
	...
	DrawPresetNameInput(T(TKEY("override_preset_label"), "Override preset"), "##LocationOverridePresetName", locationOverridePresetName, presetPath, &overridePath);

	ImGui::SameLine();
	ImGui::BeginDisabled(settings.locationOverrides.empty());
	if (ImGui::Button(T(TKEY("export_overrides"), "Export Overrides"))) {
		ExportLocationOverrides();
	}
	ImGui::EndDisabled();
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("export_overrides_tooltip"), "Export the saved override list."));
	}

	ImGui::SameLine();
	if (ImGui::Button(T(TKEY("import_overrides"), "Import Overrides"))) {
		ImportLocationOverrides();
	}
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("import_overrides_tooltip"), "Add overrides from this preset to the override list below."));
	}
...
{
	ImGui::SeparatorText(T(TKEY("full_presets"), "Full Presets"));
	DrawHintText(T(TKEY("full_presets_hint1"), "Full presets store exterior timing, the five profile tabs, and all saved location overrides."));
	DrawHintText(T(TKEY("full_presets_hint2"), "Import replaces the profile tabs and the saved override list in the current settings."));
	DrawHintText(T(TKEY("imported_changes_save_hint"), "Import changes the current settings immediately, but those changes are not saved automatically. Use the main Save Settings button to keep them after closing the game."));
	ImGui::PushID("FullPresetControls");

	const auto presetPath = GetPresetPath(fullPresetName, PresetKind::Full);
	DrawPresetNameInput(T(TKEY("full_preset_label"), "Full preset"), "##FullPresetName", fullPresetName, presetPath);

	ImGui::SameLine();
	if (ImGui::Button(T(TKEY("export_full"), "Export Full"))) {
		ExportFullPreset();
	}
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("export_full_tooltip"), "Export exterior timing, the five profile tabs, and all saved overrides."));
	}

	ImGui::SameLine();
	if (ImGui::Button(T(TKEY("import_full"), "Import Full"))) {
		ImportFullPreset();
	}
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T(TKEY("import_full_tooltip"), "Replace the profile tabs and saved override list in the current settings."));
	}
