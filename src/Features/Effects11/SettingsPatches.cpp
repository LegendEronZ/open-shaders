		...
		for (auto& entry : entries) {
			...
			for (auto& patch : entry.patches) {
				...
				for (auto& uiVar : effect.uiVariables) {
					...
					// Entries are authored either as an extender unique key or as the bare displayed
					// name; matching only one form silently drops every patch written in the other.
					if (GetUniqueKey(uiVar) != patch.variable && uiVar.displayName != patch.variable)
						continue;
