...
namespace SIE
{
	...
	void ShaderCache::TrackActiveShader(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor)
	{
		...
		if (activeShaderCaptureFramesRemaining.load(std::memory_order_relaxed) > 0 &&
			std::this_thread::get_id() == activeShaderCaptureThread) {
	...
	void ShaderCache::StartActiveShaderCaptureWindow(ActiveShaderCaptureStage a_stage)
	{
		...
		activeShaderCaptureThread = std::this_thread::get_id();
	...
	void ShaderCache::TickActiveShaderCapture(bool a_menuVisible)
	{
		...
		assert(std::this_thread::get_id() == activeShaderCaptureThread);
	...
	size_t ShaderCache::ClearActive()
	{
		...
		globals::deferred->ClearShaderCache();
		for (auto* feature : Feature::GetFeatureList()) {
			if (feature->loaded && feature->SupportsScopedShaderCacheClear())
				feature->ClearShaderCacheScoped();
		}

		QueryPerformanceCounter(&end);
		lastScopedClearCount = entries.size();
